...
//| Objective : maximize  Profit / RelativeBalanceDrawdown%          |
//| Filters   : profit > 0, relDD > 0, trades >= 100                 |
//| MT5 usage : Optimization → Custom max                            |
//+------------------------------------------------------------------+
double OnTester()
{
   double relDD  = TesterStatistics(STAT_BALANCE_DDREL_PERCENT); // 相対結余亏损%
   double profit = TesterStatistics(STAT_PROFIT);                // 净盈利
   double trades = TesterStatistics(STAT_TRADES);                // 交易笔数

   // 过滤垃圾参数：亏损 / 回撤异常 / 样本不足
   if(profit <= 0 || relDD <= 0 || trades < 100)
      return -1.0;

   // 核心目标：每单位相对回撤带来的利润（越大越好）
   return profit / relDD;
}
